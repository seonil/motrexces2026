# 모트렉스 CES 2026 쇼케이스 사이트 - 전체 텍스트 추출

## 1. Header (헤더)
- 로고: Motrex, CES 2026

---

## 2. Hero (히어로 섹션)
**메인 타이틀:**
- Innovate
- Your Own
- In-Cabin

**부가 정보:**
- LVCC, Westhall. Booth #3401
- Jan. 6–9, 2026

---

## 3. Vision (비전 섹션)
**타이틀:**
- A New Vision for
- In-Cabin Mobility

**설명:**
- At CES 2026, MOTREX showcases advanced
- in-cabin solutions and next-generation
- mobility technologies.

---

## 4. Showcase (쇼케이스 섹션)
**섹션 타이틀:**
- Our CES2026 Showcase

**탭 메뉴:**
- Driving Intelligence
- Connected Comfort
- Evolving Future

### 4-1. Driving Intelligence
**타이틀:**
- Driving Intelligence

**설명:**
- One Platform, Infinite Possibilities empowering every vehicle, from trucks to PBVs.
- MOTREX delivers seamless localization and flexible customization across every region.

### 4-2. Connected Comfort
**타이틀:**
- Connected Comfort

**설명:**
- Discover the Next-Gen Rear Seat Display, an adaptive entertainment and information display supporting both landscape and portrait rotation, providing personalized content and connected services for all passengers.

### 4-3. Evolving Future
**타이틀:**
- Evolving Future

**제품 1 - DashCam:**
- DashCam
- Advanced dual-channel recording system for comprehensive vehicle monitoring.
- Recording: Dual-Channel (Front + Rear)
- Coverage: Driving / Parking / Incident Monitoring
- Hardware: New-Generation Main PCB & Rear Camera Ass'y
- Performance: Stable, High-Quality Video
- Connectivity: App-Integrated Control

**제품 2 - AC Home EV Charger:**
- AC Home EV Charger
- Our charging solution redefines how energy is connected to life. Portable chargers provide convenience on the go, while home chargers work efficiently to build a smart and flexible EV ecosystem.
- Power Options: 7kW / 12kW / 22kW
- Compatibility: Type 1, Type 2, NACS
- Connectivity: Mobile App, RFID, Cloud
- Features: Load Balancing, IP54, 5m Cable
- Setup: Single & 3-Phase Support

**제품 3 - Portable Charger:**
- Portable Charger
- Lightweight portable charging hardware enabling flexible deployments for fleets, pop-up events, and emergency backup power.
- Power Options: 3.5kW / 2.9kW / 2.2kW
- Voltage: Single Phase, 183–277Vac
- Connectors: Type 1 / Type 2 / NACS
- Standard: IEC 62752

---

## 5. In-Cabin Innovation (인-캐빈 이노베이션 섹션)
**타이틀:**
- Innovate Your Own In-Cabin

---

## 6. Why Motrex (왜 모트렉스인가 섹션)
**섹션 타이틀:**
- Why Motrex

**카드 1:**
- Innovation Driven by
- Technology Leadership
- 25 Years of Pioneering Automotive HMI Solutions
- 4-Time Winner of CES Innovation Award
- Full-spectrum (HW/SW/UX) R&D Capabilities
- Trusted Partner to Global Leading OEMs

**카드 2:**
- Quality Excellence &
- Manufacturing Reliability
- 70+ Countries Choose Motrex for Quality and Reliability
- 10+ Manufacturing Sites Under Operation Worldwide
- Quality Assurance Aligned with Global OEM Standards
- Cutting-Edge Manufacturing and Validation Facilities

**카드 3:**
- Unified Strength
- for Future Mobility
- MOTREX Group combines expertise from Motrex EV, Motrex efm, MTR, and JUNJIN C&R to deliver solutions for next-generation mobility

---

## 7. Visit Us (방문 안내 섹션)
**타이틀:**
- Visit Us

**내용:**
- Schedule a meeting with our team to explore partnership opportunities.
- cesinfo@motrex.co.kr

---

## 8. About Us (회사 소개 섹션)
**타이틀:**
- About Us

**회사 목록:**
1. Motrex - InCabin Solution
2. Motrex EV - EV Solution
3. Motrex efm - Interior Material
4. MTR - Purpose-Built Vehicle
5. Junjin - Construction Equipment

---

## 9. Footer (푸터)
**로고 문구:**
- Beyond Driving, Beyond Imagination

**주소 및 연락처:**
- A. 56, Geumto-ro 80beon-gil, Sujeong-gu, Seongnam-si, Gyeonggi-do, Republic of Korea 13449
- T. 070-4982-6000 | F. 02-6280-1170 | E. cesinfo@motrex.co.kr

**저작권:**
- Copyright © Motrex 2026 All Rights Reserved.

---

## 검토 체크리스트

- [ ] 철자 및 문법 검토
- [ ] 대소문자 일관성 검토
- [ ] 숫자 및 날짜 정확성 검토
- [ ] 연락처 정보 정확성 검토
- [ ] 제품 사양 정확성 검토
- [ ] 브랜드명 일관성 검토 (MOTREX vs Motrex)
